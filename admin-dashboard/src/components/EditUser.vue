...
// eslint-disable-next-line
    ...
    export default class EditUser extends Vue{
        ...
        created(){
            axios({
                url:`${process.env.VUE_APP_HOST}:8000/api/admin/user`,
                method:"get",
                params:{
                ...
                this.alertTitle = "Failed to fetch the information of this user";
                this.alertShow = true;
                this.isLoading = false;
            });
        }
        ...
        mounted(){
            if(window.history && window.history.pushState){
                ...
                window.addEventListener("popstate", ()=>{
                    history.pushState("", "", document.URL);
                    this.$router.push({name: 'Home', query:{page:"users"}});
                }, false);
            }
        }
        ...
        handleSave(){
            if(this.password!=this.passwordRepeat){
                ...
                axios({
                    url: `${process.env.VUE_APP_HOST}:8000/api/admin/user/update`,
                    method: "post",
                    data:{
                        token: UserModule.userInfo.token,
                        id: this.id,
                    ...
                    if(!response.data.error){
                        ...
                        this.successMessage = "Successfully updated the information for this user!"
